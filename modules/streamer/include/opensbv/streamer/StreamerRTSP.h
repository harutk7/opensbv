//
// Created by harut on 8/15/17.
//

#ifndef PB_MAIN_STREAMERRTSP_H
#define PB_MAIN_STREAMERRTSP_H

#define RTSP_TCP_BUFFER_SIZE 1024 // buffer length for headers
#define RTSP_TCP_PORT (5445);
#define RTSP_UDP_PORT (5555);

#include <vector>
#include <cstring>
#include <pthread.h>
#include <iostream>
#include <unistd.h>
#include <set>
#include <vector>
#include <sys/types.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <memory>
#include <string>

#include "opensbv/helpers/images/ImageHelper.h" // image helper
#include "opensbv/helpers/general/GeneralHelper.h" // general helper
#include "opensbv/streamer/StreamerBase.h"
#include "opensbv/streamer/PracticalSocket.h"

/// RTSP Client Structure
struct streamerRTSPClientStruct {
    int sockFd; // client tcp fd
    int failedToSend;
    long lastActive;
    bool ready;
    string clientIp; // clientip
    unsigned short clientPort; // client udp port
    udpClientWriterStruct udpClient;
};

inline bool operator==(const streamerRTSPClientStruct& lhs, const streamerRTSPClientStruct& rhs)
{
    return lhs.sockFd == rhs.sockFd;
}
inline bool operator<(const streamerRTSPClientStruct& lhs, const streamerRTSPClientStruct& rhs)
{
    return lhs.sockFd < rhs.sockFd;
}
inline bool operator>(const streamerRTSPClientStruct& lhs, const streamerRTSPClientStruct& rhs)
{
    return lhs.sockFd < rhs.sockFd;
}
/// Listen for rtsp client worker struct
struct streamerRTSPListenWorkerStruct {
    int *sockFd; ///< Server socked fd
    pthread_mutex_t *mutex_client; ///< client mutex
    std::vector<streamerRTSPClientStruct> *clientList; ///< client list reference
};

/// RTSP streamer worker struct
struct streamerRTSPWorkerStruct {
    pthread_mutex_t *mutex_client; ///< Mutex for connecting clients
    pthread_mutex_t *mutex_stream; ///< Mutex for sending frames
    streamerDataStruct *data; ///< Data struct for streaming
    bool *isStreamReady; ///< Is stream ready or not
    std::vector<streamerRTSPClientStruct> *clientList; ///< client list reference
    bool *m_running; ///< Streaming running or not
};

/// Client rtsp command worker struct
struct streamerRTSPListenCommandWorkerStruct {
    int sockFd; ///< Server socked fd
    pthread_mutex_t *mutex_client; ///< Mutex for sending frames
    streamerRTSPClientStruct currentClient; ///< current client
    std::vector<streamerRTSPClientStruct> *clientList; ///< client list reference
};

/// RTSP Streaming class
/**
 * Stream RTSP Video to clients
 */
class StreamerRTSP: public StreamerBase {
public:
    StreamerRTSP(); ///< Constructor
    ~StreamerRTSP(); ///< Destructor

    /// Write data to stremaing
    /**
     * write data to stream buffer
     * @param data to stream
     * @param size
     */
    void Write(unsigned char *data, ssize_t size); // write to stream

    /// Run the RTSP Server
    bool Run(); // run rtsp server

    /// Stop RTSP Server
    bool Stop();

    /// Set Stream data type
    /**
     * Stream data type
     * @param dataType
     */
    void SetStreamDataType(enum stream_data_type dataType);

    static void *listenWorker(void* ptr); ///< Listen rtsp thread function
    static void *listenCommandWorker(void* ptr); ///< RTSP client command handler
    static void *streamerWorker(void* ptr); ///< RTSP streamer thread function

private:
    uint16_t                                m_tcpPort; ///< Stream server port
    int                                     m_sockFd; ///< Server socket fd
    bool                                    m_isStreamReady; ///< Is stream ready or not
    enum stream_data_type                   m_streamDataType; ///< Stream Data type
    struct                                  sockaddr_in m_serv_addr; ///< Server socket struct
    streamerRTSPListenWorkerStruct         m_listenParams; ///< Listen params for thread
    streamerRTSPWorkerStruct               m_streamerParams; ///< Streamer params for thread
    pthread_t                               m_clientListenT, m_streamerListenT; ///< Accept client and send stream threads
    pthread_mutex_t                         m_mutex_client = PTHREAD_MUTEX_INITIALIZER; ///< Mutex for control clients
    pthread_mutex_t                         m_mutex_stream = PTHREAD_MUTEX_INITIALIZER; ///< Mutex for control frames
    std::vector<unsigned char>              m_outBuf; ///< Encoded image vector
    std::vector<streamerRTSPClientStruct>  m_udpClientList; ///< UDP Client list

    /// Bind TCP Socket
    bool BindTCP();
};


#endif //PB_MAIN_STREAMERRTSP_H