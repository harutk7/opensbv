...
{
    Streamer stream;
    stream.SetStreamDataType(STREAM_DATA_BYTES);
    stream.SetStreamPort(5335);
    stream.SetStreamType(STREAM_MRTSP);
    stream.Run();

    AudioTest test;
    test.SetStreamer(&stream);
    test.Run();
...
// Open the default device
    ...
    captureDev = alcCaptureOpenDevice(curDev, 8000, AL_FORMAT_MONO16, 800);
    if (captureDev == NULL)
    {
