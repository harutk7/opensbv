...
StreamerRTSP::StreamerRTSP():StreamerBase() {
    m_streamDataType = STREAM_DATA_BYTES;
    this->m_clientListenT = 0;
    this->m_streamerListenT = 0;
}

StreamerRTSP::~StreamerRTSP() {

}

bool StreamerRTSP::BindTCP() {
    try {
        ...
        std::cout << "rtsp streaming on: " << this->m_tcpPort << std::endl;
    } catch (...) {
...
bool StreamerRTSP::Run() {
    ...
    pthread_create(&this->m_clientListenT,NULL,&StreamerRTSP::listenWorker,&m_listenParams);
    ...
    pthread_create(&this->m_streamerListenT,NULL,&StreamerRTSP::streamerWorker,&m_streamerParams);

    return true;
}

void *StreamerRTSP::listenWorker(void *ptr) {
    try {
        // parameters structure
        streamerRTSPListenWorkerStruct params = *(streamerRTSPListenWorkerStruct *)ptr;
        struct sockaddr_in cli_addr;
        socklen_t clilen;
        pthread_t listenCommandT[65535];
        int newSockFd; // new client socket fd
        streamerRTSPListenCommandWorkerStruct clientCommandParams;
        ...
        while((newSockFd = accept(*(params.sockFd), (struct sockaddr *) &cli_addr, &clilen))) {
            try {
                ...
                streamerRTSPClientStruct currentClient;
                ...
                pthread_create(&listenCommandT[newSockFd],NULL,&StreamerRTSP::listenCommandWorker,&clientCommandParams);
        ...
        std::cerr << "error in listenWorker main" << endl;
    }
}

void *StreamerRTSP::listenCommandWorker(void *ptr) {
    try {
        streamerRTSPListenCommandWorkerStruct params = *(streamerRTSPListenCommandWorkerStruct *)ptr; // params
        char receiveBuffer[RTSP_TCP_BUFFER_SIZE] = {0}; // receive buffer
        unsigned short udpPort = RTSP_UDP_PORT;
        ...
        while (true) {
            ...
            if (sel > 0) {
                ...
                ssize_t bytes = recv(params.currentClient.sockFd,receiveBuffer, RTSP_TCP_BUFFER_SIZE, 0);
...
bool StreamerRTSP::Stop() {
    try {
        ...
        std::cerr << "error in StreamerRTSP::Stop()" << std::endl;
    }
}

void StreamerRTSP::SetStreamDataType(enum stream_data_type dataType) {
...
void *StreamerRTSP::streamerWorker(void *ptr) {
    try {
        // parameters struct
        streamerRTSPWorkerStruct params = *(streamerRTSPWorkerStruct *)ptr;
